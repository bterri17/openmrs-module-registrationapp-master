        ...
        if (!errors.hasErrors()) {
            try {
                ...
                InfoErrorMessageUtil.flashInfoMessage(request.getSession(),
                        ui.message("fbregistrationapp.editContactInfoMessage.success", patient.getPersonName()));

                return "redirect:" + returnUrl;
            }
            catch (Exception e) {
                log.warn("Error occurred while saving patient's contact info", e);
                session.setAttribute(UiCommonsConstants.SESSION_ATTRIBUTE_ERROR_MESSAGE, "fbregistrationapp.save.fail");
            }
