package org.openmrs.module.fbregistrationapp.fragment.controller;
...
import org.openmrs.module.fbregistrationapp.RegistrationAppUiUtils;
import org.openmrs.module.fbregistrationapp.RegistrationAppUtils;
import org.openmrs.module.fbregistrationapp.action.AfterPatientCreatedAction;
...
public class RegisterPatientFragmentController {
    ...
    public FragmentActionResult submit(UiSessionContext sessionContext, @RequestParam(value="appId") AppDescriptor app,
        ...
        // now create the fb_registration encounter, if configured to do so
        ...
        InfoErrorMessageUtil.flashInfoMessage(request.getSession(), ui.message("fbregistrationapp.createdPatientMessage", patient.getPersonName()));
    ...
    private Encounter buildRegistrationEncounter(Patient patient, Date registrationDate, UiSessionContext sessionContext, AppDescriptor app, EncounterService encounterService) {
        ...
        // if no fb_registration encounter type specified, we aren't configured to create an encounter
    ...
    private void checkForIdentifierExceptions(Exception ex, Errors errors) {
        ...
        if (ex instanceof PatientIdentifierException) {
            if (ex instanceof InvalidCheckDigitException) {
                errors.reject("fbregistrationapp.error.identifier.invalidCheckDigit");
            }
            else if (ex instanceof DuplicateIdentifierException) {
                errors.reject("fbregistrationapp.error.identifier.duplicate");
            }
            else {
                errors.reject("fbregistrationapp.error.identifier.general");
            }
        }
        else if (ex instanceof EmptyIdentifierPoolException) {
            errors.reject("fbregistrationapp.error.identifier.emptyIdentifierPool");
        }
    }
